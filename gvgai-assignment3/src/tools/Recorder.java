...
public static double[] featureExtract(StateObservation obs){
        ...
        double[] feature = new double[3 * 14 + 4 + 1];
        
        int avatarX = (int) (obs.getAvatarPosition().x / 25);
        
        int[][] map = new int[3][14];
        int leftNPC = 0, rightNPC = 0;
        // Extract features
        LinkedList<Observation> allobj = new LinkedList<>();
        ...
        if( obs.getNPCPositions()!=null )
            for(ArrayList<Observation> l : obs.getNPCPositions()) {
            	...
            	for (Observation o : l) {
            		int x = (int) (o.position.x / 25);
            		if (x < avatarX) leftNPC++;
            		else if (x > avatarX) rightNPC++;
            		else if (o.itype == 5) hasBomb = true;
            	}
            }
        ...
        for(Observation o : allobj){
            ...
            if (Math.abs(x - avatarX) <= 1)
            	map[x - avatarX + 1][y] = o.itype;
        }
        for(int y=0; y<14; y++)
            for(int x=0; x<3; x++)
        ...
        // 4 states
        feature[42] = obs.getGameTick();
        feature[43] = hasBomb ? 1.0 : 0.0;
        feature[44] = leftNPC;
        feature[45] = rightNPC;
...
//    }
	...
	public static Instances datasetHeader(){
	    ...
	    att = new Attribute("leftNPC"); attInfo.addElement(att);
	    att = new Attribute("rightNPC"); attInfo.addElement(att);
	    //class
	    FastVector classes = new FastVector();
	    classes.addElement("0");
	    classes.addElement("1");
