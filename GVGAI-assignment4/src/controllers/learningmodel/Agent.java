...
public class Agent extends AbstractPlayer {
	...
	// 决定了每次模拟时看向未来的步数, 即每次蒙特卡罗方法中从当前局势出发前进的最大步数
	// 如果在SIMULATION_DEPTH范围内就达到游戏出口, 则停止迭代
	...
	// dataset中所能存放最大Instance的个数, 超过该值从序列前端删除
	// 每个Instance是一个时间节点上提取的特征
	protected double m_gamma = 0.99;
	// 控制未来回报对当前影响的大小, γ=1时认为未来和当前一样重要, y=0时忽略未来回报
	...
	private Instances simulate(StateObservation stateObs, StateHeuristic heuristic, QPolicy policy) {
		...
		for (; depth < SIMULATION_DEPTH; depth++) {
			try {
				...
				int action_num = policy.getAction(features, depth);
	...
	private void learnPolicy(StateObservation stateObs, int maxdepth, StateHeuristic heuristic) {
		...
		int iter = 0;
		while (iter++ <= 25 // truem_timer.remainingTimeMillis() > SIMULATION_DEPTH*10
		) {
