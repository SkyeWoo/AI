...
public class MTDDecider implements Decider {
	...
	/**
	 * Creates a new MTDDecider optionally using a secondary heuristic to compare
	 * 
	 * @param maximizer
	 *            Is this player maximizing the heuristic score?
	 * @param searchTimeSec
	 ...
	 */
	public MTDDecider(boolean maximizer, int searchTimeMSec, int maxdepth, boolean usemtd) {
		...
		for (d = 1; d <= maxdepth; d++) {
			...
			for (ActionValuePair a : actions) {
				...
				try {
					...
					a.value = value;
					/*
					 * if (maximizer) { alpha = Math.max(alpha, value); } else
					 */
	...
	/**
	 * Main MTD(f) search algorithm which recursively uses alpha-beta with varying
	 * bounds to compute the minimax value of the root
	 * 
	 * @param root
	 ...
	 * @return The best guess for the minimax value given the available search depth
	...
	private int AlphaBetaWithMemory(State state, int alpha, int beta, int depth, int color) throws OutOfTimeException {
		...
		/**
		 * If we are not at a low depth (have at least more recursive calls below us)
		 * then we are called infrequently enough that we can afford to check if we are
		 * out of time
		 */
		if (depth > 4) {
			if (times_up())
		...
		if (node != null && node.depth >= depth) {
			...
			case EXACT_VALUE:
				return node.value;
			/*
			 * case UPPERBOUND: if (node.value > alpha) alpha = node.value; break; case
			 * LOWERBOUND: if (node.value < beta) beta = node.value; break;
			 */
			}
		}
	 ...
	 * @param color
	 *            The current "color" i.e. whether we are maximizing the heuristic
	 *            or the negative heuristic
	 * @return The value that we stored
	 */
	private int saveAndReturnState(State state, int alpha, int beta, int depth, int value, int color) {
	...
	/**
	 * Returns a random action from among the best actions in the given list NOTE:
	 * this assumes the list is already sorted with the best move first, and that
