...
/**
 * This class represents an AI Decider that uses a MiniMax algorithm. We use
 * alpha-beta pruning, but besides that we're pretty vanilla.
 * 
...
public class MiniMaxDecider implements Decider {
	 ...
	 * 
	 * @param maximize
	 *            Are we maximizing or minimizing on this turn? True if the former.
	 * @param depth
	 *            The depth to which we should analyze the search space.
	 */
	 ...
	 * 
	 * @param state
	 *            The start State for our search.
	...
	public Action decide(State state) {
		...
		for (Action action : state.getActions()) {
			try {
				...
				float newValue = this.AlphaBetaSearch(newState, 1, !this.maximize, Float.NEGATIVE_INFINITY,
		...
		// ((OthelloState) state).debugprint();
		return bestActions.get(0);
	}

	/**
	 * The true implementation of the Alpha-Beta algorithm! Thoroughly commented for
	 * your convenience.
	 * 
	 * @param state
	 ...
	 *         the specified depth.
	 */
	public float AlphaBetaSearch(State state, int depth, boolean maximize, float alpha, float beta) {
		...
		// return count < 20 ? finalize(state, state.heuristic()) : ((OthelloState)
		// state).heuristic2();
		// Have we reached the end of the line?
		if (depth == this.depth)
			// Return the heuristic value
			return state.heuristic();
		// return count < 20 ? state.heuristic() : ((OthelloState) state).heuristic2();
		...
		if (maximize) {
			for (Action action : test) {
				...
				try {
					...
					float newValue = AlphaBetaSearch(childState, depth + 1, !maximize, alpha, beta);
	 ...
	 * 
	 * @param state
	 *            The State we are hashing.
	 * @param value
	 *            The value that State has.
