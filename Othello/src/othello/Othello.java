		...
		for (byte i = 0; i < 8; i++)
			for (byte j = 0; j < 8; j++)
				...
				case 'X':
					...
					g.fillOval(1 + i * Square_L, 1 + j * Square_L, Square_L - 1, Square_L - 1);
					break;
				case 'O':
...
public class Othello extends JFrame {
	...
	public Othello(int whichPlayer, int nummilli, int maxDepth) {
		...
		/*
		 * String[] moves =
		 * "d3 c3 c4 c5 b5 d2 c2 f3 f5 c6 f4 a5 a6 a7 d1 e3 d6 a4 g3 e2 b4 c1 b1 e1 f1 a3 c7 f2 g2 g4 b3 h1"
		 * .split(" "); // h4 b2
		 * 
		 * for (String s: moves) { Point p = sToM(s); start =
		 * start.childOnMove((byte)p.x, (byte)p.y); }
		 * 
		 * boolean computerIsPlayerOne = false; boolean computerMovesFirst = false;
		 */
		boolean computerIsMaximizer = (whichPlayer == 1);
		boolean computerMovesFirst = computerIsMaximizer;

		// gamePanel = new GamePanel(new MiniMaxDecider(computerIsMaximizer, maxDepth),
		// start, computerMovesFirst);
		...
		JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, gamePanel, status);
