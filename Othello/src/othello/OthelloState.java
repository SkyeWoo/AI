...
public class OthelloState implements State {
	...
	// The dimension of this board. Since we're using shorts, must be less than or
	// equal to 8.
	...
	/**
	 * Lookup table that has all results for lines from moves. First index is the
	 * current line... 2^16; all possible shorts. Second index is the index of the
	 * move... dimension possibilities. Third index is the player making the move...
	 * 2 players.
	 */
	...
	// Lookup table storing the random long corresponding to each state of each
	// square on the board
	private static int[][][] zobristHashes = new int[dimension][dimension][3];

	private final double[][] g_Weight = new double[][] {
	...
	/**
	 * Generates random bitstrings which correspond to each state of each board
	 * square. Used for hashing the state.
	 */
	 ...
	 * 
	 * @param line
	 *            The line, as so far specified.
	 * @param points
	 *            The points, as so far counted.
	 * @param depth
	 *            The depth we have so far traversed.
	 */
	 ...
	 * 
	 * @param line
	 *            The line to draw from.
	 * @param index
	 *            The index on the line we want.
	 ...
	 * 
	 * @param x
	 *            The x-coordinate of the spot we want to retrieve.
	 * @param y
	 *            The y-coordinate of the spot we want to retrieve.
	 * @return ' ' if the spot is empty; 'O' if the spot has player 1; 'X' if the
	 *         spot has player 2.
	 */
	 ...
	 * 
	 * @param left
	 *            The spots to the left of the center spot.
	 * @param center
	 *            The center spot.
	 * @param right
	 *            The spots to the right of the center spot.
	 * @param index
	 *            The index at which the center spot is located.
	 ...
	 * 
	 * @param line
	 *            The original state of the line.
	 * @param newLine
	 *            The new state of the line.
	 * @param index
	 *            The index of the actual move.
	 ...
	 * 
	 * @param line
	 *            The line to generate the value for.
	 */
	private static void generateLookupAndMoveValue(short line) {
		...
		for (byte i = 0; i < dimension; i++) {
			...
			if (spot != 0) {
				if (DEBUG)
					System.out.println("Can't move on index " + i + "! Already occupied.");
				if (DEBUG)
					System.out.println(lineToString(line) + " resolves to itself");
			...
			if (firstByte > 1 && j < dimension) {
				...
				while (true) {
					...
					if (DEBUG)
						System.out.println("Flips will occur on right!");
					j--;
					if (first)
						for (; j > i; j--)
			...
			if (firstByte > 1 && j > 0) {
				...
				while (true) {
					...
					// The flips shall occur!
					if (DEBUG)
						System.out.println("Flips will occur on left!");
					j++;
					if (first)
						for (; j < i; j++)
							p2Left = setSpotOnLine(p2Left, j, (byte) 3);
					else
						for (; j < i; j++)
							p1Left = setSpotOnLine(p1Left, j, (byte) 2);
					break;
				}
			}
			...
			if (DEBUG)
				System.out.println(lineToString(line) + " resolves to " + lineToString(p1));
			if (DEBUG)
				System.out.println(lineToString(line) + " resolves to " + lineToString(p2));
		...
		if (DEBUG)
			System.out.println(lineToString(line) + " allows p1 " + lineToString(reverseLine(p1MoveLine)));
		if (DEBUG)
			System.out.println(lineToString(line) + " allows p2 " + lineToString(reverseLine(p2MoveLine)));
	}
	...
	/**
	 * Get the index of a spot within a line. Different boards will use different
	 * indexes for their lines.
	 * 
	 * @param board
	 ...
	 * 
	 * @param board
	 *            The bit-board to set the line on.
	 * @param line
	 *            The actual value to set that line to.
	 * @param x
	 *            The x-coordinate of the spot the move was on, so we can figure out
	 *            the index of the line to change.
	 * @param y
	 *            The y-coordinate of the spot the move was on; see above.
	 */
	...
	public boolean moveIsValid(byte x, byte y, boolean player) {
		...
		for (byte i = 0; i < 4; i++) {
			...
			if (getFlippedSpots(lines[i], newLine, index) != 0)
				return true;
		}
		// No flips, so illegal
		return false;
	 ...
	 * @param boardIndex
	 *            The index of the type of board.
	 * @param x
	 *            The x-coordinate of the spot.
	 * @param y
	 *            The y-coordinate of the spot.
	...
	/**
	 * Generate bit-boards representing the validity of moves for each player. 00 =
	 * invalid; 10 = valid. 2 bits so we can use point lookup table.
	 */
	public void generateMoveBoards() {
		...
		for (byte i = 0; i < dimension; i++) {
			for (byte j = 0; j < dimension; j++) {
				this.p1MoveBoard[i] = (short) (this.p1MoveBoard[i] | getSpotOnBoardAt(hBoards[1], (byte) 0, i, j)
						| getSpotOnBoardAt(vBoards[1], (byte) 1, i, j) | getSpotOnBoardAt(dBoard1s[1], (byte) 2, i, j)
						| getSpotOnBoardAt(dBoard2s[1], (byte) 3, i, j));
				this.p2MoveBoard[i] = (short) (this.p2MoveBoard[i] | getSpotOnBoardAt(hBoards[0], (byte) 0, i, j)
						| getSpotOnBoardAt(vBoards[0], (byte) 1, i, j) | getSpotOnBoardAt(dBoard1s[0], (byte) 2, i, j)
	 ...
	 * 
	 * @param x
	 *            The x-coordinate to set at.
	 * @param y
	 *            The y-coordinate to set at.
	 * @param skip
	 *            A board to skip setting.
	 * @param value
	 *            The value to set.
	 */
	...
	/**
	 * Execute a move and mutate our bit-boards to represent this. We assume that
	 * the other player is making this move.
	 * 
	 * @param x
	 *            The x-coordinate of the move.
	 * @param y
	...
	private void executeMove(boolean player, byte x, byte y) {
		...
		for (byte i = 0; i < 4; i++) {
			...
			for (byte j = 0; j < dimension; j++) {
				...
				if (getSpotOnLine(flippedSpots, j) == 0)
					continue;
				// If it's changed, then flip it on all other boards!
				if (i == 0)
					this.flipValueOnBoards(x, j, i);
				else if (i == 1)
					this.flipValueOnBoards(j, y, i);
				else if (i == 2)
					this.flipValueOnBoards(j, (byte) (x + y - j), i);
				else if (i == 3)
					this.flipValueOnBoards(j, (byte) (j - x + y), i);
			}
	 ...
	 * 
	 * @param board
	 *            The board to get the stability metric for.
	 * @return The stability metric for the provided board.
	 */
	private float getStabilitySumForBoard(short[] board) {
		float metric = 0;
		for (short line : board)
			metric += stabilityTable[line + 32768];
		return metric;
	}

	/**
	 * Returns the difference between the stability metric for player 1 and player
	 * 2.
	 * 
	 * @return The difference between the stability metric for player 1 and player 2
	 */
	private float stabilityDifferential() {
		return getStabilitySumForBoard(hBoard) + getStabilitySumForBoard(vBoard) + getStabilitySumForBoard(dBoard1)
	...
	/**
	 * Returns the difference between the amount of moves player one and player two
	 * have available.
	 * 
	 * @return the difference between the amount of moves player one and player two
	 *         have available.
	 */
	...
	/**
	 * Get the difference between the number of corners player 1 and player 2
	 * occupy.
	 * 
	...
	public void debugprint() {
		for (int i = 0; i < dimension; i++) {
			for (int j = 0; j < dimension; j++)
				System.out.print(getSpotOnLine(hBoard[i], (byte) j));
			System.out.println();
		}
	}
	...
	public float heuristic() {
		// System.out.printf("%f %f %f %f\n",this.pieceDifferential(),
		// this.moveDifferential(), this.cornerDifferential(),
		...
		// break;
		// }
		// return -2 * this.pieceDifferential() + 8 * this.moveDifferential() + 300 *
		// this.cornerDifferential()
		// + winconstant;
		// 1 * this.stabilityDifferential() +
		float sum = 0;
		for (int i = 0; i < dimension; i++)
			for (byte j = 0; j < dimension; j++)
				if (getSpotOnLine(hBoard[i], j) == 2)
					sum += g_Weight[i][j];
				else if (getSpotOnLine(hBoard[i], j) == 3)
					sum -= g_Weight[i][j];

		return sum + this.moveDifferential();
	}
	...
	public float heuristic2() {
		...
		return this.pieceDifferential() + 8 * this.moveDifferential() + 300 * this.cornerDifferential()
				+ this.stabilityDifferential() + winconstant;
	}
	...
	public List<Action> getActions() {
		...
		if (actions.isEmpty())
			actions.add(new OthelloAction(this.move, (byte) -1, (byte) -1));
		return actions;
	}
	...
	public Status getStatus() {
		...
		for (short line : p1MoveBoard)
			if (line != 0)
				return Status.Ongoing;
		for (short line : p2MoveBoard)
			if (line != 0)
				return Status.Ongoing;
		// Game over!
	...
	private void computeHashValue() {
		...
		for (int i = 0; i < dimension; i++) {
			for (int j = 0; j < dimension; j++) {
				...
				// and convert it into 0, 1 or 2 (the state as an index into the zobristHash
				// table
				int state = 0;
				switch (value) {
				case 0:
	...
	/**
	 * Returns an integer hash code for this OthelloState. If two objects are equal,
	...
	/**
	 * Checks the equivalence between this OthelloState and another Object. Will
	 * screw up if passed anything that isn't an OthelloState.
	 * 
	 * @param other
